...
import openai
import requests
from run_scienceqa.demos import prompt_ck

# from easyocr.DBNet.assets.ops.dcn.setup import modules
...
class solver:
    ...
    def predict_modules(self):
        ...
        # 换gptapi
        modules = get_azure_chat_response(messages, self.api_key, self.policy_engine, self.policy_temperature,self.policy_max_tokens)
    ...
    def knowledge_retrieval(self):
        ...
        knowledge = get_azure_chat_response(messages, self.api_key, self.kr_engine, self.kr_temperature, self.kr_max_tokens)
    ...
    def query_generator(self):
        ...
        query = get_azure_chat_response(messages, self.api_key, self.qg_engine, self.qg_temperature, self.qg_max_tokens)
    ...
    def bing_search(self):
        ...
        def google_search(self):
            ...
            # execute the module (call Google Custom Search API and get the responses)
            if query is not None and query != "":
                result = call_google_search(endpoint, google_api_key, google_search_engine_id, query, count)
            else:
                result = None

            responses = parse_google_result(result)

            if len(responses) > 0 and responses[0] != "":
                response += f"\n\nGoogle search response: {responses}"
                response = response.strip()

            # update the cache
            self.cache["response"] = response
            self.cache["google_search:input"] = query
            self.cache["google_search:output"] = responses
            return query, responses

        # Helper functions to call Google Custom Search API
        def call_google_search(endpoint, api_key, search_engine_id, query, count):
            url = f"https://www.googleapis.com/customsearch/v1?q={query}&key={api_key}&cx={search_engine_id}&num={count}"
            response = requests.get(url)
            return response.json() if response.status_code == 200 else None

        def parse_google_result(result):
            if result and "items" in result:
                return [item['snippet'] for item in result['items']]
            return []
    ...
    def build_prompt_for_ck_chameleon(self):
        # get the input
        question_text = self.get_question_text()
        metadata = self.get_metadata()
        response = self.cache["response"] if "response" in self.cache else ""
        solution = self.cache["solution"] if "solution" in self.cache else ""

        # build the prompt
        demo_prompt = prompt_ck.prompt.strip() # WARNING: this is the prompt for chameleon
        if response != "":
            test_prompt = f"Question: {question_text}\n\nMetadata: {metadata}\n\n{response}\n\nSolution: {solution}\n\nCheck: "
        else:
            test_prompt = f"Question: {question_text}\n\nMetadata: {metadata}\n\nSolution: {solution}\n\nCheck: "
    ...
    def solution_generator(self):
        ...
        while count < patience and not success:
            ...
            solution = get_azure_chat_response(messages, self.api_key, self.sg_engine, _temperature, self.sg_max_tokens)
    ...
    def check_generator(self):
        # get the module input
        if self.model == "chameleon":
            test_prompt, full_prompt = self.build_prompt_for_ck_chameleon()
        else:
            test_prompt, full_prompt = self.build_prompt_for_sg_cot()
        ...
        while count < patience and not success:
            ...
            check = get_azure_chat_response(messages, self.api_key, self.sg_engine, _temperature, self.sg_max_tokens)
        ...
        self.cache["check_generator:input"] = test_prompt
        self.cache["check_generator:output"] = check
        return test_prompt, check


    def answer_generator(self):
        # get the module input
        output = self.cache["check"]
